...
import '../styles/VirtualTour.css';  // Import CSS for responsive mobile controls
...
function UnityPlayer() {
  ...
  const [isPaused, setIsPaused] = useState(false); // Untuk pause overlay
  ...
  const tryCreateInstance = async () => {
    ...
    const config = {
      ...
      devicePixelRatio: window.devicePixelRatio || 2, // Use device pixel ratio for crisp display
    };
  ...
  // Setup canvas focus for keyboard input and Pointer Lock
  useEffect(() => {
    if (started && canvasRef.current) {
      ...
      // Re-focus canvas when clicking on it and handle Pointer Lock
      const handleCanvasClick = () => {
        canvas.focus();
        
        // Jika sedang paused, jangan request pointer lock
        if (isPaused) return;
        
        // Request Pointer Lock after user gesture (click)
        // This fixes: "A user gesture is required to request Pointer Lock"
        if (canvas.requestPointerLock) {
          canvas.requestPointerLock().catch((err) => {
            // Pointer Lock failed - this is OK, just log it
            console.log('Pointer Lock request:', err.message || err);
          });
        }
      };
      ...
      const handlePointerLockChange = () => {
        if (document.pointerLockElement === canvas) {
          ...
          setIsPaused(true); // Pause game dan tampilkan menu
        }
      };
      
      // Listen for Pointer Lock errors
      const handlePointerLockError = (err) => {
        console.log('Pointer Lock error (can be ignored):', err);
      };
    ...
    <div className="unity-container" style={{ width: '100%', maxWidth: 1200, margin: '0 auto' }}>
      {!started ? (
        ...
        <div className="unity-player" style={{ position: 'relative' }}>
          {/* Pause Menu Overlay (when ESC pressed or pointer lock exits) */}
          {isPaused && loadingProgress >= 1 && !loadingError && (
            <div className="unity-pause-menu" style={{
              position: 'absolute',
              ...
              <div style={{ 
                ...
                <p style={{ 
                  ...
                  Tekan <span style={{ 
                    ...
                    fontWeight: 'bold',
                    color: '#fff'
                  }}>ESC</span> kapan saja untuk pause
                </p>
                <div style={{ display: 'flex', gap: 16, justifyContent: 'center' }}>
                  <button 
                    onClick={async () => {
                      if (canvasRef.current) {
                        ...
                        try {
                          ...
                          // Jika error "The user has exited the lock before this request was completed.", abaikan saja
                          if (
                            err &&
                            (err.message === 'The user has exited the lock before this request was completed.' ||
                              String(err).includes('The user has exited the lock before this request was completed.'))
                          ) {
                            // Suppress this error silently
                            return;
                          }
                          // Tampilkan notifikasi jika gagal karena bukan user gesture
                          NotificationManager.error(
                            'Pointer Lock Gagal',
                            'Klik tombol Mulai untuk mengaktifkan kontrol mouse.'
                          );
                        }
                      }
                    }}
                    ...
                    onMouseEnter={(e) => {
                      ...
                      e.currentTarget.style.background = 'linear-gradient(135deg, #5a5a5a 0%, #3a3a3a 100%)';
                    }}
                    onMouseLeave={(e) => {
                      e.currentTarget.style.transform = 'translateY(0)';
                      e.currentTarget.style.boxShadow = '0 4px 15px rgba(0, 0, 0, 0.3)';
                      e.currentTarget.style.background = 'linear-gradient(135deg, #4a4a4a 0%, #2a2a2a 100%)';
                    }}
                  >
                    <span style={{ fontSize: '1.3rem', color: '#fff', filter: 'none' }}>▶️</span>
                    <span>Mulai</span>
                  </button>
                  <button 
                    onClick={stopUnity}
                    ...
                    style={{
                      ...
                      alignItems: 'center',
                      gap: '8px',
                      opacity: isQuitting ? 0.6 : 1
                    }}
                    onMouseEnter={(e) => {
                      if (!isQuitting) {
                        e.currentTarget.style.transform = 'translateY(-2px)';
                        e.currentTarget.style.boxShadow = '0 6px 20px rgba(255, 255, 255, 0.15)';
                        e.currentTarget.style.background = 'linear-gradient(135deg, #434343 0%, #2a2a2a 100%)';
                      }
                    }}
                    onMouseLeave={(e) => {
                      if (!isQuitting) {
                  ...
                  >
                    ...
                    <span>{isQuitting ? 'Keluar...' : 'Stop'}</span>
                  </button>
                </div>
              </div>
            </div>
          ...
          {/* Mobile-friendly controls - ONLY VISIBLE ON MOBILE */}
          <div className="mobile-controls-container" style={{ marginTop: 12 }}>
            {/* D-Pad controls for mobile */}
            <div className="mobile-dpad-controls" style={{ 
              display: 'grid', 
            ...
            <div className="mobile-action-buttons" style={{ display: 'flex', gap: 8, justifyContent: 'center', flexWrap: 'wrap' }}>
              <button 
                onTouchStart={(e) => { e.preventDefault(); handleMobileControl('up'); }}
                onClick={() => handleMobileControl('up')} 
                className="btn-secondary btn-mobile-action"
                style={{
            ...
            <div style={{
              ...
              <div className="mobile-instruction" style={{ marginBottom: 6 }}>
                💡 <strong>Mobile:</strong> Swipe pada layar atau gunakan tombol D-Pad
              </div>
              <div className="desktop-instruction" style={{ marginBottom: 6 }}>
                ⌨️ <strong>Desktop/Laptop:</strong> Klik canvas lalu gunakan keyboard WASD atau arrow keys
              </div>
              <div style={{ marginTop: 4, fontSize: '0.8rem', color: '#777' }}>
                🎮 Tekan <strong>ESC</strong> untuk pause/keluar dari mode kontrol
              </div>
              {lastInput && (
                <div style={{ 
                  marginTop: 8, 
