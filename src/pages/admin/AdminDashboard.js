...
function AdminDashboard() {
  ...
  // HOUSE TYPES STATE
  const [houseTypesList, setHouseTypesList] = useState([]);
  ...
  // HOUSE LAYOUTS STATE (currently not displayed in UI)
  // eslint-disable-next-line no-unused-vars
  ...
  /* ===============================
     HOUSE TYPES HANDLING
  =============================== */
  useEffect(() => {
    if (isAuthenticated && activeSection === 'housetypes') loadHouseTypes();
  }, [isAuthenticated, activeSection]);

  const loadHouseTypes = async () => {
    const res = await cmsApi.getHouseTypes(true); // include inactive
    if (res.status === 'success') setHouseTypesList(res.data || []);
  };

  const handleCreateHouseType = async () => {
    if (!newHouseType.name || !newHouseType.type_category) {
      NotificationManager.warning('⚠️ Incomplete', 'Name and category are required');
      return;
    }

    const res = await cmsApi.createHouseType(newHouseType);
    if (res.status === 'success') {
      NotificationManager.success('✅ Success', 'House type created successfully');
      setNewHouseType({
        name: '',
        description: '',
        price_start: 0,
      ...
      NotificationManager.error('❌ Error', res.message || 'Failed to create house type');
    }
  };

  // Update house type function (for future use - edit functionality)
  // eslint-disable-next-line no-unused-vars
  const handleUpdateHouseType = async (id, data) => {
    const res = await cmsApi.updateHouseType(id, data);
    if (res.status === 'success') {
      NotificationManager.success('✅ Success', 'House type updated successfully');
      setEditingHouseType(null);
      loadHouseTypes();
    } else {
      NotificationManager.error('❌ Error', res.message || 'Failed to update house type');
    }
  };

  const handleDeleteHouseType = async (id) => {
    if (!window.confirm('Are you sure you want to delete this house type?')) return;
    const res = await cmsApi.deleteHouseType(id);
    if (res.status === 'success') {
      NotificationManager.success('✅ Success', 'House type deleted');
      loadHouseTypes();
    }
  };

  const handleToggleHouseTypeActive = async (id) => {
    const res = await cmsApi.toggleHouseTypeActive(id);
    if (res.status === 'success') {
      NotificationManager.success('✅ Success', 'House type status toggled');
      loadHouseTypes();
    }
  };

  const addFeatureToNewHouse = () => {
    if (featureInput.trim()) {
      setNewHouseType({
        ...newHouseType,
        features: [...newHouseType.features, featureInput.trim()]
      });
      setFeatureInput('');
    }
  };

  const removeFeatureFromNewHouse = (index) => {
    const updatedFeatures = newHouseType.features.filter((_, i) => i !== index);
    setNewHouseType({ ...newHouseType, features: updatedFeatures });
  };
  ...
  // Layout management functions (for future use)
  // eslint-disable-next-line no-unused-vars
    ...
    <div className="admin-dashboard">
      ...
      <aside className="admin-sidebar">
        ...
        <ul className="section-list">
          {[
            ...
            { key: 'housetypes', label: 'Tipe Rumah' },
      ...
      <main className="admin-content">
        <div className="content-header">
          ...
          {activeSection !== 'news' && activeSection !== 'qna' && activeSection !== 'faq' && activeSection !== 'messages' && activeSection !== 'social' && activeSection !== 'housetypes' && activeSection !== 'layouts' && activeSection !== 'activity' && (
        ...
        {/* ================= HOUSE TYPES SECTION ================= */}
        {activeSection === 'housetypes' && (
          <>
            <div className="admin-section">
              <div className="faq-header">
                <h2>Manajemen Tipe Rumah</h2>
                <div className="faq-summary">
                  <span className="badge badge-total">Total: {houseTypesList.length}</span>
                  <span className="badge badge-active">Active: {houseTypesList.filter(h => h.is_active).length}</span>
                  <span className="badge badge-inactive">Inactive: {houseTypesList.filter(h => !h.is_active).length}</span>
                </div>
              </div>

              {/* Add New House Type Form */}
              <div className="create-form">
                <h3>Tambah Tipe Rumah Baru</h3>
                ...
                <div className="form-row">
                  <div className="form-group">
                    ...
                    <input
                      ...
                      placeholder="Rumah Modern Minimalis"
                    />
                  </div>
                  <div className="form-group">
                    <label>Kategori *</label>
                    <select
                      value={newHouseType.type_category}
                      onChange={(e) => setNewHouseType({...newHouseType, type_category: e.target.value})}
                    >
                ...
                <div className="form-group">
                  ...
                  <textarea
                    ...
                    onChange={(e) => setNewHouseType({...newHouseType, description: e.target.value})}
                    placeholder="Deskripsi singkat tentang tipe rumah..."
                  />
                </div>
                ...
                <div className="form-row">
                  <div className="form-group">
                    ...
                    <input
                      type="number"
                      value={newHouseType.price_start}
                      onChange={(e) => setNewHouseType({...newHouseType, price_start: parseFloat(e.target.value)})}
                  ...
                  <div className="form-group">
                    ...
                    <input
                      type="text"
                      value={newHouseType.land_size}
                      onChange={(e) => setNewHouseType({...newHouseType, land_size: e.target.value})}
                      placeholder="10x15 m"
                    />
                  </div>
                </div>
                ...
                <div className="form-row">
                  <div className="form-group">
                    ...
                    <input
                      type="text"
                      value={newHouseType.building_size}
                      onChange={(e) => setNewHouseType({...newHouseType, building_size: e.target.value})}
                      placeholder="120 m²"
                    />
                  </div>
                  <div className="form-group">
                    ...
                    <input
                      type="text"
                      value={newHouseType.image_url}
                      onChange={(e) => setNewHouseType({...newHouseType, image_url: e.target.value})}
                      placeholder="/assets/house-modern.jpg"
                    />
                  </div>
                </div>
                ...
                <div className="form-row">
                  <div className="form-group">
                    ...
                    <input
                      type="number"
                      value={newHouseType.bedrooms}
                      onChange={(e) => setNewHouseType({...newHouseType, bedrooms: parseInt(e.target.value)})}
                    />
                  </div>
                  <div className="form-group">
                    ...
                    <input
                      type="number"
                      value={newHouseType.bathrooms}
                      onChange={(e) => setNewHouseType({...newHouseType, bathrooms: parseInt(e.target.value)})}
                    />
                  </div>
                  <div className="form-group">
                    <label>Lantai</label>
                    <input
                      type="number"
                      value={newHouseType.floors}
                      onChange={(e) => setNewHouseType({...newHouseType, floors: parseInt(e.target.value)})}
                    />
                  </div>
                  <div className="form-group">
                    <label>Carport</label>
                    <input
                      type="number"
                      value={newHouseType.carport}
                      onChange={(e) => setNewHouseType({...newHouseType, carport: parseInt(e.target.value)})}
                    />
                  </div>
                </div>
                ...
                <div className="form-group">
                  ...
                  <div style={{display: 'flex', gap: '10px', marginBottom: '10px'}}>
                    <input
                      ...
                      placeholder="Contoh: Smart Home Ready"
                      onKeyPress={(e) => {
                        if (e.key === 'Enter') {
                          e.preventDefault();
                          addFeatureToNewHouse();
                        }
                      }}
                    />
                    <button 
                      type="button"
                      onClick={addFeatureToNewHouse}
                      className="btn-secondary"
                  ...
                  <div style={{display: 'flex', flexWrap: 'wrap', gap: '8px'}}>
                    {newHouseType.features.map((feature, idx) => (
                      <span 
                        key={idx}
                        style={{
                          background: '#333',
                      ...
                      >
                        ...
                        <button
                          type="button"
                          onClick={() => removeFeatureFromNewHouse(idx)}
                          style={{
                ...
                <div className="form-row">
                  <div className="form-group">
                    ...
                    <input
                      type="number"
                      value={newHouseType.display_order}
                      onChange={(e) => setNewHouseType({...newHouseType, display_order: parseInt(e.target.value)})}
                    />
                  </div>
                  <div className="form-group">
                    <label>Status</label>
                    <select
                      value={newHouseType.is_active}
                      onChange={(e) => setNewHouseType({...newHouseType, is_active: parseInt(e.target.value)})}
                    >
                      <option value={1}>Active</option>
                      <option value={0}>Inactive</option>
                    </select>
                ...
                <button onClick={handleCreateHouseType} className="btn-primary">
                  Tambah Tipe Rumah
                </button>
              </div>

              {/* House Types List */}
              {houseTypesList.length === 0 ? (
                <p className="empty-state">Belum ada tipe rumah</p>
              ) : (
                <div className="faq-table-wrapper">
                  <table className="faq-table">
                    <thead>
                    ...
                    <tbody>
                      {houseTypesList.map(house => (
                        <tr key={house.id}>
                          ...
                          <td>
                            <span className="category-badge">{house.type_category}</span>
                          </td>
                          <td>
                            {house.price_start > 0 ? (
                              <div>
                                Rp {(house.price_start / 1000000).toFixed(0)}jt
                              </div>
                            ) : '-'}
                          </td>
                          <td>
                            <div style={{fontSize: '0.85rem'}}>
                              <div>🏠 {house.land_size || '-'}</div>
                              <div>📐 {house.building_size || '-'}</div>
                              <div>🛏️ {house.bedrooms}KT | 🚿 {house.bathrooms}KM</div>
                            </div>
                          </td>
                          <td>
                            <div style={{fontSize: '0.85rem', maxWidth: '200px'}}>
                              {Array.isArray(house.features) && house.features.length > 0 ? (
                                house.features.slice(0, 3).map((f, i) => (
                                  <div key={i}>• {f}</div>
                                ))
                              ) : '-'}
                              {Array.isArray(house.features) && house.features.length > 3 && (
                                <div style={{opacity: 0.7}}>+{house.features.length - 3} more</div>
                              )}
                            </div>
                          </td>
                          <td className="text-center">{house.display_order}</td>
                          <td>
                            <span className={`status-badge ${house.is_active ? 'active' : 'inactive'}`}>
                          ...
                          <td>
                            <div className="action-buttons">
                              <button
                                className="btn-secondary"
                                onClick={() => handleToggleHouseTypeActive(house.id)}
                              ...
                              <button
                                className="btn-delete"
                                onClick={() => handleDeleteHouseType(house.id)}
